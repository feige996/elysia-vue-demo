...
export const createStorage = (): IStorage | null => {
    switch (env.STORAGE_TYPE as StorageType) {
        ...
        case 'oss':
            if (!env.OSS_REGION || !env.OSS_ACCESS_KEY_ID || !env.OSS_ACCESS_KEY_SECRET || !env.OSS_BUCKET) {
                console.warn('OSS storage is configured but missing required environment variables');
                return null;
            }
        ...
        case 'cos':
            if (!env.COS_SECRET_ID || !env.COS_SECRET_KEY || !env.COS_BUCKET || !env.COS_REGION) {
                console.warn('COS storage is configured but missing required environment variables');
                return null;
            }
