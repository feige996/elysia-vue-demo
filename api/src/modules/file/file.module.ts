...
if (!storage) {
  console.warn(
    'File storage is not configured. File upload functionality will be unavailable.',
  );
}
...
export const fileModule = new Elysia({
    ...
    },
    {
      ...
      beforeHandle: () => {
        if (!fileService) {
          return {
            status: 503,
            payload: { message: 'File storage is not configured' },
          };
        }
      },
    },
  ...
  .get(
    ...
    async (ctx) => {
      ...
      const controller = createFileController(fileService);
      const response = await controller.getUrl(
        ctx.query as Record<string, string | undefined>,
        ctx.request,
      );
      ctx.set.status = response.status;
