...
import { deleteFileSchema, uploadBodySchema, validateFile } from './dto/file.dto';
...
export const createFileController = (fileService: FileService) => ({
    upload: async (body: unknown, request: Request) => {
        ...
        if (!parsedBody.success) {
            return failByKey(requestId, ErrorKey.VALIDATION_ERROR, parsedBody.error.issues[0]?.message ?? 'Invalid upload payload');
        }
    ...
    delete: async (query: Record<string, string | undefined>, request: Request) => {
        const { requestId } = ensureRequestContext(request);
    ...
    getUrl: async (query: Record<string, string | undefined>, request: Request) => {
        const { requestId } = ensureRequestContext(request);

        if (!query.key) {
            return failByKey(requestId, ErrorKey.VALIDATION_ERROR, 'File key is required');
        }
