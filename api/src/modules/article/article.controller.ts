...
export const createArticleController = (
  ...
  listAll: async (query: unknown, request: Request) => {
    ...
    const list = await articleRepository.findAll(
  ...
  list: async (query: unknown, request: Request) => {
    ...
    const result = await articleRepository.findPage(
      page,
      pageSize,
  ...
  create: async (body: unknown, request: Request) => {
    ...
    if (!parsedBody.success) {
      return failByKey(
        requestId,
        ErrorKey.VALIDATION_ERROR,
        parsedBody.error.issues[0]?.message ?? 'Invalid article payload',
      );
    }
  ...
  removeBatch: async (body: unknown, request: Request) => {
    ...
    const deletedCount = await articleRepository.deleteByIds(
      parsedBody.data.ids,
    );
    return {
      status: 200,
