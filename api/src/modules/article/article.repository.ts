...
const buildKeywordFilter = (keyword?: string) => {
    ...
    const text = `%${keyword}%`;
    return or(ilike(articlesTable.title, text), ilike(articlesTable.author, text));
};
...
export class ArticleRepository {
    ...
    async findPage(page: number, pageSize: number, keyword?: string) {
        ...
        const totalRows = await db.select({ total: count() }).from(articlesTable).where(filter);
        return {
            list: rows,
    ...
    async update(id: number, input: UpdateArticleInput) {
        ...
        const rows = await db.update(articlesTable).set(input).where(eq(articlesTable.id, id)).returning({
            id: articlesTable.id,
            title: articlesTable.title,
            author: articlesTable.author,
        });
    ...
    async deleteById(id: number) {
        const rows = await db.delete(articlesTable).where(eq(articlesTable.id, id)).returning({ id: articlesTable.id });
        return rows.length > 0;
    }

    async deleteByIds(ids: number[]) {
        if (ids.length === 0) return 0;
        const rows = await db.delete(articlesTable).where(inArray(articlesTable.id, ids)).returning({ id: articlesTable.id });
        return rows.length;
    }
}
