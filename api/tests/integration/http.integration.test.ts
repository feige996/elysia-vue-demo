...
const userRepository = {
  ...
  async findPage(page: number, pageSize: number, keyword?: string) {
    const source = !keyword
      ...
      : users.filter(
          (user) =>
            user.account.includes(keyword) || user.name.includes(keyword),
        );
...
describe('HTTP integration', () => {
  ...
  it('supports refresh token rotation and logout', async () => {
    ...
    const refreshResponse = await sendJson(
    ...
    const oldRefreshReuseResponse = await sendJson(
      'http://localhost/api/auth/refresh',
      'POST',
      {
        refreshToken: loginPayload.data?.refreshToken ?? '',
    ...
    const logoutResponse = await sendJson(
  ...
  it('returns permission codes and menu tree for current user', async () => {
    ...
    const permissionResponse = await app.handle(
      new Request('http://localhost/api/permissions/current', {
        headers: authHeaders,
      }),
    );
  ...
  it('supports user and article CRUD endpoints', async () => {
    ...
    const createUserResponse = await sendJson(
      'http://localhost/api/users',
      'POST',
      { account: 'bob', name: 'Bob', role: 'editor' },
      authHeaders,
    );
    expect(createUserResponse.status).toBe(201);

    const usersPageResponse = await app.handle(
      new Request('http://localhost/api/users?page=1&pageSize=20', {
        headers: authHeaders,
      }),
    );
    const usersPagePayload = (await usersPageResponse.json()) as {
      data?: { list: Array<{ id: number; account: string }> };
    };
    const createdUser = usersPagePayload.data?.list.find(
      (item) => item.account === 'bob',
    );
    expect(createdUser?.account).toBe('bob');

    const updateUserResponse = await sendJson(
      `http://localhost/api/users/${createdUser?.id ?? 0}`,
      'PUT',
      { name: 'Bobby' },
      authHeaders,
    );
    ...
    const deleteUserResponse = await sendJson(
      `http://localhost/api/users/${createdUser?.id ?? 0}`,
      'DELETE',
      undefined,
      authHeaders,
    ...
    const createArticleResponse = await sendJson(
      'http://localhost/api/articles',
      'POST',
      { title: 'New Article', author: 'Bob' },
      authHeaders,
    );
    expect(createArticleResponse.status).toBe(201);

    const allArticlesResponse = await app.handle(
      new Request('http://localhost/api/articles/all'),
    );
    const allArticlesPayload = (await allArticlesResponse.json()) as {
      data?: Array<{ id: number; title: string }>;
    };
    const createdArticle = allArticlesPayload.data?.find(
      (item) => item.title === 'New Article',
    );
    expect(createdArticle?.title).toBe('New Article');

    const batchDeleteResponse = await sendJson(
      'http://localhost/api/articles',
      'DELETE',
      { ids: [createdArticle?.id ?? 0] },
      authHeaders,
    );
    const batchDeletePayload = (await batchDeleteResponse.json()) as {
      data?: { deleted: number };
    };
