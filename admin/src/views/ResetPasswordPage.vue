...
import { resetPasswordMethod } from '../api/modules/auth';
...
const schema = z.object({
  ...
  verifyCode: z.string().length(6, '请输入 6 位验证码'),
...
const submit = async () => {
  ...
  loading.value = true;
  try {
    await resetPasswordMethod(parsed.data);
    successText.value = '密码已重置，请返回登录';
  } catch (error) {
    errorText.value = getMappedErrorMessage(error, '重置密码失败');
  } finally {
    loading.value = false;
  }
};
...
<template>
  <div class="auth-page">
    <NCard title="重置密码" :bordered="false" class="auth-card">
