...
<template>
  <div class="storage-page-root">
    <NCard title="存储管理中心" :bordered="false" class="storage-page-card">
      <NScrollbar class="storage-page-scrollbar" trigger="hover">
        <div class="storage-page-inner">
          <NForm label-placement="left" label-width="140">
            ...
            <template v-if="isLocal">
              <NFormItem label="Local 基础目录">
                <NInput
                  v-model:value="form.local.baseDir"
                  placeholder="uploads"
                />
              </NFormItem>
            ...
            <NFormItem label=" ">
              <div style="display: flex; gap: 8px; align-items: center">
                <NButton type="primary" :loading="saving" @click="saveConfig"
                  >保存配置</NButton
                >
                <NButton :loading="testing" @click="testConfig"
                  >联通性测试</NButton
                >
          ...
          <NCard
            ...
            title="敏感信息预览（脱敏）"
            size="small"
            class="masked-preview-card"
          >
            <pre class="masked-preview-pre">{{
              JSON.stringify(masked, null, 2)
            }}</pre>
          </NCard>
          <NTag v-if="errorText" type="error" class="storage-error-tag">{{
            errorText
          }}</NTag>
        </div>
      </NScrollbar>
...
.storage-page-card :deep(.n-card-header) {
  flex-shrink: 0;
}

.storage-page-card :deep(.n-card-content) {
  flex: 1;
  min-height: 0;
  min-width: 0;
  overflow: hidden;
